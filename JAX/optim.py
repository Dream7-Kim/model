...
weight = model_weight.modelf0(var_weight)
...
def model(var):    
    ...
    # 添加共振态 数据
    for i in range(num):
        up_phif001 = phif001.T * BW(var[0 + 12*i], var[1 + 12*i], Kp, Km) * BW(var[2 + 12*i], var[3 + 12*i], Pip, Pim) * phase(var[4 + 12*i] - var[5 + 12*i], var[6 + 12*i])
        ...
        up = up + (up_phif001 + up_phif021) * phase(var[5 + 12*i], var[7 + 12*i])
    # print(up.shape)
    ...
    # 添加共振态 蒙卡
    for i in range(num): 
        low_phif001 = phif001MC.T * BW(var[0 + 12*i], var[1 + 12*i], KpMC, KmMC) * BW(var[2 + 12*i], var[3 + 12*i], PipMC, PimMC) * phase(var[4 + 12*i] - var[5 + 12*i], var[6 + 12*i])
        ...
        low = low + (low_phif001 + low_phif021) * phase(var[5 + 12*i], var[7 + 12*i])
